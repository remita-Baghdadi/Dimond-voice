// import 'dart:async';
// import 'package:dimondvoice/provider/archive_api_provider.dart';
// import 'package:dimondvoice/provider//books_db_provider.dart';

// import 'package:dimondvoice/models/BookModel.dart';

// class Repository {
//   List<Source> sources = <Source>[
//     archiveApiProvider,
//   ];

//   List<Cache> caches = <Cache>[DatabaseHelper()];

//   Future<List<Book>> fetchBooks(int offset, int limit) async {
//     List<Book> books;
//     books = await caches[0].getBooks(offset, limit);
//     if (books.isEmpty) {
//       books = await sources[0].fetchBooks(offset, limit);
//       caches[0].saveBooks(books);
//     }
//     return books;
//   }

//   Future<List<Book>> topBooks() async {
//     List<Book> books;
//     books = await sources[0].topBooks();
//     return books;
//   }

//   Future<List<Chapter>> fetchAudioFiles(String? bookId) async {
//     List<Chapter> audiofiles;
//     audiofiles = await caches[0].fetchAudioFiles(bookId);
//     if (audiofiles.isEmpty) {
//       audiofiles = await sources[0].fetchAudioFiles(bookId);
//       caches[0].saveAudioFiles(audiofiles);
//     }
//     return audiofiles;
//   }
// }

// abstract class Source {
//   Future<List<Book>> fetchBooks(int offset, int limit);
//   Future<List<Book>> topBooks();
//   Future<List<Chapter>> fetchAudioFiles(String? bookId);
// }

// abstract class Cache {
//   Future saveBooks(List<Book> books);
//   Future saveAudioFiles(List<Chapter> audiofiles);
//   Future<List<Book>> getBooks(int offset, int limit);
//   Future<List<Chapter>> fetchAudioFiles(String? bookId);
// }
